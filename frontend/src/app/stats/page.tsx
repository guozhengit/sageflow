...
import { documentApi, statsApi } from '@/lib/services'
import { useAuth } from '@/hooks/useAuth'
import { useApi } from '@/hooks/useApi'
import type { UserStats } from '@/lib/services'
...
export default function StatsPage() {
  ...
  const { data: stats, isLoading: statsLoading } = useApi<UserStats>(() => statsApi.getUser())
  const { data: documents, isLoading: docsLoading } = useApi(() => documentApi.list())

  const loading = statsLoading || docsLoading

  const memberSince = useMemo(() => {
    if (!user?.created_at) return 'N/A'
    try { return new Date(user.created_at).toLocaleDateString() } catch { return 'N/A' }
  }, [user?.created_at])

  const documentCount = documents?.length ?? 0
  const knowledgeBaseBytes = documents?.reduce((sum, doc) => sum + doc.file_size, 0) ?? 0
  ...
  const statCards = [
    ...
    { label: 'Documents Uploaded', value: documentCount, icon: '📄', color: 'from-purple-500 to-purple-600' },
    { label: 'Knowledge Base Size', value: formatFileSize(knowledgeBaseBytes), icon: '💾', color: 'from-orange-500 to-orange-600' },
  ]
                  ...
                  <div key={idx} className="bg-sage-900/50 rounded-xl border border-sage-800 p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${card.color} flex items-center justify-center`}>
                        <span className="text-white text-lg font-bold">{typeof card.value === 'number' ? card.value : card.value[0]}</span>
                      </div>
                    </div>
                ...
                <div className="bg-sage-900/50 rounded-xl border border-sage-800 p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-white">{user.preferences?.plan || 'N/A'}</p>
                    </div>
                  </div>
                </div>
              )}
